...
```{r}
# nyt_raw %>% filter(fips >= 66) %>% group_by(state) %>% slice(1)
...
Raw dataset consists of `r number(nrow(measures))` records of `r number(length(unique(measures$state)))` states with measures implemented between `r min(measures$date)` to `r max(measures$date)`. 
