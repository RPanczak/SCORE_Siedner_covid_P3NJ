...
nyt_first %>% 
  ...
  summarise(median = median(cases_cumulative),
            p25 = quantile(cases_cumulative, prob = 0.25),
            p75 = quantile(cases_cumulative, prob = 0.75))
```
...
nyt_first %>% 
  filter(date == date_first) %>% 
  summarise(mean = mean(cases_day_perc))
```
